...
export async function generatePDF(options: PDFExportOptions): Promise<Blob> {
  ...
  try {
    ...
    const PrintTemplate = await import(
      "../components/custom-ui/literature/export/PrintTemplate.svelte"
    );
    ...
    const bibliographyContent = tempContainer.querySelector(
    ...
    if (bibliographyContent) {
      ...
      const referenceEntries =
        bibliographyContent.querySelectorAll(".reference-entry");
      const header = bibliographyContent.querySelector(
        ".bibliography-header"
      ) as HTMLElement;
      ...
      if (header) {
        try {
          ...
          console.warn(
            "Failed to render header, continuing without it:",
            error
          );
        }
      }
      ...
      for (let i = 0; i < referenceEntries.length; i++) {
        ...
        try {
          ...
          let entryImgHeight =
            (entryCanvas.height * entryImgWidth) / entryCanvas.width;
          ...
          console.warn(
            `Failed to render reference entry ${i + 1}, skipping:`,
            error
          );
      ...
      const footer = bibliographyContent.querySelector(
        ".bibliography-footer"
      ) as HTMLElement;
      if (footer) {
        ...
        const footerImgHeight =
          (footerCanvas.height * footerImgWidth) / footerCanvas.width;
