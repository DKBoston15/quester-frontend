...
>
	...
	>
		...
		{#if editor?.isEditable}
			...
			<div
				class={cn(
					'absolute -top-8 right-0 flex items-center gap-1 rounded border bg-background p-1 opacity-0 transition-opacity',
					...
					<DropdownMenu.Content align="start" alignOffset={-90} class="mt-1 overflow-auto text-sm">
						<DropdownMenu.Item
							onclick={() => {
								if (node.attrs.title === null || node.attrs.title.trim() === '')
									updateAttributes({
										title: $_('editor.media.audioCaption')
									});
							}}
						>
